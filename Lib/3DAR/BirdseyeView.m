...
- (id) initWithLocations:(NSArray *)_locations around:(CLLocation *)_centerLocation radiusInPixels:(CGFloat)_radiusInPixels
{
    ...
    if (self) 
    {
        ...
        fovInDegrees = 50.0;
        maxRangeInDegrees = 1.15;
        minRangeInDegrees = 0.01;
        rangeInDegrees = ((maxRangeInDegrees - minRangeInDegrees) / 4.0) + minRangeInDegrees;
...
- (void)drawRect:(CGRect)rect 
{
    ...
    CGFloat colorValue = 0.85;
    CGRect circleFrame = CGRectMake(outlineWidth/2, 
                                    outlineWidth/2, 
                                    2*radiusInPixels-outlineWidth, 
                                    2*radiusInPixels-outlineWidth);

    CGContextSetRGBStrokeColor(context, colorValue, colorValue, colorValue, 0.4);
    CGContextSetLineWidth(context, outlineWidth);
    CGContextStrokeEllipseInRect(context, circleFrame);

    CGContextSetRGBFillColor(context, colorValue, colorValue, colorValue, 0.1);
    ...
    for (id loc in locations)
    {
        ...
        CLLocationCoordinate2D coord = ((CLLocation*)loc).coordinate;
